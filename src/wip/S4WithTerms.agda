...
lookupᵥ : ∀ {{𝔐 : 𝔎}} {Ξ A w} → w ᵏ⊩⋆ Ξ → Ξ ∋ A
                              → w ᵏ⊩ A
lookupᵥ γ i = lookupAll γ i


-- Soundness
↓ : ∀ {Δ Γ M A} → Δ ⁏ Γ ⊢ M ∷ A
                → Δ ⁏ Γ ⊨ A
↓ (ᵐᵛ x # i)             = λ δ γ → lookupᵥ (sem⋆ δ) (proj∋₂ i)
↓ (ᵛ x # i)              = λ δ γ → lookupᵥ γ (proj∋₂ i)
↓ (ƛ_∙_ {A} {B} x 𝒟)     = λ δ γ → return {A ⊃ B} (λ η k →
                             ↓ 𝒟 (ᵐᵏmov⋆ η δ) (ᵏmov⋆ η γ , k))
↓ (_$_ {A} {B} 𝒟 ℰ)      = λ δ γ → bind {A ⊃ B} {B} (↓ 𝒟 δ γ) (λ η f →
                             f idₐ (↓ ℰ (ᵐᵏmov⋆ η δ) (ᵏmov⋆ η γ)))
↓ (_,_ {A} {B} 𝒟 ℰ)      = λ δ γ → return {A ∧ B} (↓ 𝒟 δ γ , ↓ ℰ δ γ)
↓ (π₁ {A} {B} 𝒟)         = λ δ γ → bind {A ∧ B} {A} (↓ 𝒟 δ γ) (λ η p → proj₁ p)
↓ (π₂ {A} {B} 𝒟)         = λ δ γ → bind {A ∧ B} {B} (↓ 𝒟 δ γ) (λ η p → proj₂ p)
↓ tt                     = λ δ γ → return {𝒯} tt
↓ {Δ} (⌜_⌝ {A} {M} 𝒟)    rewrite lem₄ Δ ⁻¹
                         = λ δ γ → return {□ A} (λ η →
                             ᵐsubₜₘ {ξ = map proj₁ Δ} (syn⋆ (ᵐᵏmov⋆ η δ)) 𝒟
                           , ᵐsub {ξ = map proj₁ Δ} (syn⋆ (ᵐᵏmov⋆ η δ)) 𝒟
                           , ↓ 𝒟 (ᵐᵏmov⋆ η δ) ∅)
↓ (⌞_⌟_∙_ {A} {C} 𝒟 x ℰ) = λ δ γ → bind {□ A} {C} (↓ 𝒟 δ γ) (λ η f →
                             ↓ ℰ (ᵐᵏmov⋆ η δ , f idₐ) (ᵏmov⋆ η γ))
...
mutual
  ...
  ↓ᵤ {𝒯 }    {M} 𝒟 = return {𝒯} tt
  ↓ᵤ {□ A}   {M} 𝒟 = λ η f →
                       let N , ℰ = f (ᵐwk² idᵣ²) (λ η′ →
                                     ᵐᵛ "MFRESH"
                                   , (ᵐᵛ "MFRESH" # ᵐlookupᵣ² η′ zero)
                                   , ↓ᵤ (ᵐᵛ "MFRESH" # ᵐlookupᵣ² η′ zero)) in
                       ⌞ M ⌟ "MFRESH" ∙ N , ⌞ renₙₜ² η 𝒟 ⌟ "MFRESH" ∙ ℰ

  ↑ᵤ : ∀ {A Δ Γ} → Δ ⁏ Γ ᵏ⊩ A
                 → Σ Tm (λ M → Δ ⁏ Γ ⊢ₙₘ M ∷ A)
  ↑ᵤ {ᵗᵛ x}  k = k idᵣ² (λ η p → p)
  ↑ᵤ {A ⊃ B} k = k idᵣ² (λ η f → let M , 𝒟 = ↑ᵤ (f (wk² idᵣ²) (↓ᵤ (ᵛ "RFRESH" # zero))) in
                                  ƛ "RFRESH" ∙ M , ƛ "RFRESH" ∙ 𝒟)
  ↑ᵤ {A ∧ B} k = k idᵣ² (λ η p → let M , 𝒟 = ↑ᵤ (proj₁ p) in
                                  let N , ℰ = ↑ᵤ (proj₂ p) in
                                  (M , N) , (𝒟 , ℰ))
  ↑ᵤ {𝒯}     k = k idᵣ² (λ η t → tt , tt)
  ↑ᵤ {□ A}   k = k idᵣ² (λ η f → let M , 𝒟 = syn (f idᵣ²) in
                                  ⌜ M ⌝ , ⌜ 𝒟 ⌝)


ᵐidᵥ : ∀ {Δ Γ} → Δ ⁏ Γ ᵐᵏ⊩⋆ map proj₂ Δ
ᵐidᵥ {∅}           = ∅
ᵐidᵥ {Δ , (x , A)} = ᵐᵏmov⋆ (ᵐwk² idᵣ²) ᵐidᵥ , (ᵐᵛ x , (ᵐᵛ x # zero) , ↓ᵤ (ᵐᵛ x # zero))

idᵥ : ∀ {Γ Δ} → Δ ⁏ Γ ᵏ⊩⋆ map proj₂ Γ
idᵥ {∅}           = ∅
idᵥ {Γ , (x , A)} = ᵏmov⋆ (wk² idᵣ²) idᵥ , ↓ᵤ (ᵛ x # zero)

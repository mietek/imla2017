...
ᵐᵏmov : ∀ {{𝔐 : 𝔎}} {A w w′} → w′ ≥ w → w ᵐᵏ⊩ A
                             → w′ ᵐᵏ⊩ A
ᵐᵏmov {A} η (𝒟 , k) = ᵐren ᵐ⌊ η ⌋ₐ 𝒟 , ᵏmov {A} η k
...
↓ : ∀ {Δ Γ A} → Δ ⁏ Γ ⊢ A
              → Δ ⁏ Γ ⊨ A
↓ (ᵐᵛ i)            = λ δ γ → lookupᵥ (sem⋆ δ) i
↓ (ᵛ i)             = λ δ γ → lookupᵥ γ i
↓ (ƛ {A} {B} 𝒟)     = λ δ γ → return {A ⊃ B} (λ η k →
...
↓ (⌜_⌝ {A} 𝒟)       = λ δ γ → return {□ A} (λ η →
                        ᵐsub (syn⋆ (ᵐᵏmov⋆ η δ)) 𝒟 , ↓ 𝒟 (ᵐᵏmov⋆ η δ) ∅)
↓ (⌞_⌟ {A} {C} 𝒟 ℰ) = λ δ γ → bind {□ A} {C} (↓ 𝒟 δ γ) (λ η f →
...
mutual
  ...
  ↓ᵤ {𝒯}     𝒟 = return {𝒯} tt
  ↓ᵤ {□ A}   𝒟 = λ η f → ⌞ renₙₜ² η 𝒟 ⌟ (f (ᵐwk² idᵣ²) (λ η′ →
                   ᵐᵛ (ᵐlookupᵣ² η′ zero) , ↓ᵤ (ᵐᵛ (ᵐlookupᵣ² η′ zero))))
  ...
  ↑ᵤ {𝒯}     k = k idᵣ² (λ η t → tt)
  ↑ᵤ {□ A}   k = k idᵣ² (λ η f → ⌜ syn (f idᵣ²) ⌝)


ᵐidᵥ : ∀ {Δ Γ} → Δ ⁏ Γ ᵐᵏ⊩⋆ Δ
ᵐidᵥ {∅}     = ∅
ᵐidᵥ {Δ , A} = ᵐᵏmov⋆ (ᵐwk² idᵣ²) ᵐidᵥ , (ᵐᵛ zero , ↓ᵤ (ᵐᵛ zero))

idᵥ : ∀ {Γ Δ} → Δ ⁏ Γ ᵏ⊩⋆ Γ
idᵥ {∅}     = ∅
idᵥ {Γ , A} = ᵏmov⋆ (wk² idᵣ²) idᵥ , ↓ᵤ (ᵛ zero)

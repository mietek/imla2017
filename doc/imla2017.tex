...
\end{alignat*}
The proof of $\monoe$ proceeds by induction, using monotonicity of stack membership in the $\var$ and $\mvar$ case.
...
\begin{alignat*}{3}
  & \graftin ~\:~ \rlap{$\Gamma \; \Delta \es \Xis ~\r~ A \in \Xis ~\r~ \Gamma \; \Delta \e A$}\\
  & \graftin~(\xi \, M)~\tops              && ~=~ M\\
...
\end{alignat*}
It is important to note that the $\quotnf$ normal form wraps a closed, non-normal form derivation, $\O \; \Delta \e A$.  This matches the definition of convertibility for the $\uplambdabox$-calculus given by Pfenning and Davies, which is not congruent under the $\Box$ introduction rule.  In other words, code may be quoted and become data; such data may be evaluated and become code.
...
We are now ready to define a function that maps proof trees to values within the canonical model, $\reflectc~M$, mutually with an inverse mapping, $\reifyc~\kappa$, which reads back the resulting values as derivations in normal form.  Both definitions proceed by induction on the type of the derivation.
\begin{alignat*}{5}

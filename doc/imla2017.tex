...
Proof trees, or derivations in the $\uplambdabox$-calculus, are formalised as an indexed type family, $\Gamma \; \Delta \e A$, after Altenkirch and Reus\cite{AltenkirchR99}.  This ensures that only well-typed terms are representable:
\begin{align*}
  & \prftree
     {i \: A \in \Gamma}
     {\var~i \: \Gamma \; \Delta \e A}\quad
  & \prftree
     {i \: A \in \Delta}
     {\mvar~i \: \Gamma \; \Delta \e A}\\[.5\baselineskip]
  & \prftree
     {M \: \Gamma, A \; \Delta \e B}
     {\lam~M \: \Gamma \; \Delta \e A \rf B}\quad
  & \prftree
     {M \: \O \; \Delta \e A}
     {\quot~M \: \Gamma \; \Delta \e \Box A}\\[.5\baselineskip]
  & \prftree
     {M \: \Gamma \; \Delta \e A \rf B}{N \: \Gamma \; \Delta \e A}
     {\app~M~N \: \Gamma \; \Delta \e B}\quad
  & \prftree
     {M \: \Gamma \; \Delta \e \Box A}{N \: \Gamma \; \Delta, A \e C}
     {\eval~M~N \: \Gamma \; \Delta \e C}
\end{align*}
Multiple terms in a single context can be represented as a pointwise lifting of syntactic entailment to stacks of types, $\Gamma \; \Delta \es \Xi$, constituting a \emph{syntactic environment}.  The letters $\gamma, \delta, \xi$ refer to syntactic environments.

Note that the $\quot$ form wraps a closed derivation, $\O \; \Delta \e A$, or a proof tree in which only modal assumptions are made.  We view the collection of modal assumptions, $\Delta$, as a typed specification of modal holes left in a given derivation.  These holes must be plugged in order for a derivation to fully evaluate.  Our evaluation function requires us to supply a syntactic environment of \emph{modal evidence,} containing a closed derivation for each modal assumption.
...
\begin{alignat*}{2}
  & \monoe   && ~\:~ \Gamma \; \Delta \leqII \Gamma' \; \Delta' ~\r~ \Gamma \; \Delta \e A ~\r~ \Gamma' \; \Delta' \e A\\
  & \monoes  && ~\:~ \Gamma \; \Delta \leqII \Gamma' \; \Delta' ~\r~ \Gamma \; \Delta \es \Xis ~\r~ \Gamma' \; \Delta' \es \Xis
\end{alignat*}
The proof of monotonicity of syntactic entailment, $\monoe$, proceeds by induction, using monotonicity of stack membership, $\monoin$, in the $\var$ and $\mvar$ case.
...
We define proof trees in normal form, $\Gamma \; \Delta \enf A$, mutually with proof trees in \emph{neutral form,} $\Gamma \; \Delta \ene A$.  Neutral forms correspond to terms that are blocked from evaluation by the presence of an assumption, as described by Altenkirch\cite{Altenkirch93}:
\begin{align*}
  & \prftree
     {M \: \Gamma, A \; \Delta \enf B}
     {\lamnf~M \: \Gamma \; \Delta \enf A \rf B}\quad
  & \prftree
     {i \: A \in \Gamma}
     {\varne~i \: \Gamma \; \Delta \ene A}\\[.5\baselineskip]
  & \prftree
     {M \: \O \; \Delta \e A}
     {\quotnf~M \: \Gamma \; \Delta \enf \Box A}\quad
  & \prftree
     {i \: A \in \Delta}
     {\mvarne~i \: \Gamma \; \Delta \ene A}\\[.5\baselineskip]
  & &\prftree
     {M \: \Gamma \; \Delta \ene A \rf B}{N \: \Gamma \; \Delta \enf A}
     {\appne~M~N \: \Gamma \; \Delta \ene B}\\[.5\baselineskip]
  & \prftree
     {M \: \Gamma \; \Delta \ene A}
     {\nenf~M \: \Gamma \; \Delta \enf A}\quad
  & \prftree
...
\begin{alignat*}{2}
  & \monoenf && ~\:~ \Gamma \; \Delta \leqII \Gamma' \; \Delta' ~\r~ \Gamma \; \Delta \enf A ~\r~ \Gamma' \; \Delta' \enf A\\
  & \monoene && ~\:~ \Gamma \; \Delta \leqII \Gamma' \; \Delta' ~\r~ \Gamma \; \Delta \ene A ~\r~ \Gamma' \; \Delta' \ene A
\end{alignat*}
It is important to note that the $\quotnf$ normal form wraps a closed, non-normal form derivation, $\O \; \Delta \e A$, for we do not wish to evaluate under $\quot$.  This matches the definition of convertibility for the $\uplambdabox$-calculus given by Pfenning and Davies, which is not congruent under the $\Box$ introduction rule.
...
We are now ready to define a function that maps proof trees to values within the canonical model, $\reflectc~M\mkern-3mu$, mutually with an inverse mapping, $\reifyc~\kappa$, which reads back the resulting values as derivations in normal form.  Both definitions proceed by induction on the type of the derivation.
\begin{alignat*}{5}
  & \reflectc ~\:~ \rlap{$\Gamma \; \Delta \ene A ~\r~ \Gamma \; \Delta \ee A$}\\
  ...
  & \reifyc ~\:~ \rlap{$\Gamma \; \Delta \ee A ~\r~ \Gamma \; \Delta \enf A$}\\
